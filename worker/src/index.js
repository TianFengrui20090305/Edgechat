...
app.get("/api/auth/session", async (c) => {
  ...
  const freshSession = {
    ...
    avatarUrl: user.results[0].avatar_key
      ? `/files/${encodeURIComponent(user.results[0].avatar_key)}`
      : "",
  };
...
app.patch("/api/me/profile", async (c) => {
  ...
  const merged = {
    ...
    displayName,
    avatarUrl: avatarKey
      ? `/files/${encodeURIComponent(avatarKey)}`
  ...
  return c.json({
    users: results.map((row) => ({
      ...
      avatarUrl: row.avatar_key
        ? `/files/${encodeURIComponent(row.avatar_key)}`
        : "",
    })),
  });
});
  ...
  return c.json({
    ...
    dms: dmsResult.results.map((row) => ({
      ...
      otherUser: {
        ...
        avatarUrl: row.other_avatar_key
          ? `/files/${encodeURIComponent(row.other_avatar_key)}`
          : "",
      },
    })),
  });
...
app.get("/api/ws/:kind/:id", async (c) => {
  ...
  const stub = c.env.CHANNEL_ROOM.get(
    c.env.CHANNEL_ROOM.idFromName(`${kind}:${id}`),
  );
